...
func (cluster *Cluster) ReDiscover () error {
	var err error
	for _, node := range cluster.nodes {
		fmt.Printf("asking node %s for an updated slot(s) map\n", node.address)
		err = cluster.update(node)
		if err == nil {
			go cluster.handleUpdate()
			fmt.Printf("slot map update queued with data from %s\n", node.address)
			// You can return since any node has the slot map, you only really need to ask one in a functioning cluster
			break;
		}
	}
	return err
